import { FraccionamientoService } from "../../services/maestros/fraccionamiento.js";
...
export class FraccionamientoController {
  ...
  getById = async (req, res) => {
    const { id } = req.params;
    try {
      const data = await fraccionamientoService.getById(id);
  ...
  create = async (req, res) => {
    ...
    try {
      const data = await fraccionamientoService.create(estado);
  ...
  delete = async (req, res) => {
    ...
    try {
      const data = await fraccionamientoService.delete(id);
  ...
  update = async (req, res) => {
    ...
    const { estado } = req.body;
    try {
      const data = await fraccionamientoService.update(id, estado);
